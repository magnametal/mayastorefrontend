...
@Component({
  selector: 'app-recuperacionCode',
  templateUrl: './recuperacionCode.component.html',
  styleUrls: ['./recuperacionCode.component.scss'],
  encapsulation: ViewEncapsulation.None,
})
export class RecuperacionCodeComponent {
  constructor(
  ...
  verify(){
    ...
    if (code!='') {
      this.api.apiPostRequest(`login/recuperacion/codigo`, {
        email: this.email,
        code: code
      }).subscribe({
        next: (resp: any) => {
          if (resp.ok) {
            this.router.navigate(['recuperacion/key/'+this.email+'/'+resp.key]);
          }else{
